...
double sjf(t_carpincho* c){
	...
	if(c->ultimo_estimado == -1){
		c->ultimo_estimado =  config_get_double_value(config, "ESTIMACION_INICIAL");
...
void hacer_tarea(void* args){
	...
	bool buscar_carpincho(void* carpincho_en_lista) {return ((t_carpincho*)carpincho_en_lista)->pid == carpincho->pid;}
	...
	while(carpincho->estado == EXEC){
		if(string_equals_ignore_case(carpincho->esperando, "VACIO")){
			...
			}else if(cod_op == MEMFREE || cod_op == MEMREAD || cod_op == MEMWRITE){
			...
			finalizar_instruccion_sem(carpincho,inicio_exec);
			return;
		}
...
void* atender_op_memoria(int retorno, uint32_t rta_memalloc, int cod_op, t_carpincho* carpincho){
	...
	if(conexion != -1){
		...
		if(cod_mem == RMEMALLOC){ //TODO poner cod de conexions q no lo toma
...
int post_semaforo(t_carpincho* carpincho){
	...
	if(list_any_satisfy(lista_semaforos,buscar_semaforo)){
		t_semaforo* objetivo = list_find(lista_semaforos,buscar_semaforo);
		objetivo->valor++;
		if(list_find(carpincho->tomando,buscar_semaforo) != NULL){
			list_remove_by_condition(carpincho->tomando,buscar_semaforo);
			list_remove_by_condition(objetivo->tomado_por,buscar_carpincho);
		}
		logear(SEM_POSTED,carpincho->pid,1,carpincho->instruccion->nombre_sem_io);
		return desbloquear_en_espera_sem(carpincho);
	}else{
		logear(SEM_NOEXISTE,carpincho->pid,1,carpincho->instruccion->nombre_sem_io);
		return NO_EXISTE_SEMAFORO;
	}
}
...
void finalizar_instruccion_sem(t_carpincho* carpincho, int inicio_exec){ //TODO
	bool buscar_bloqueando(void* sem_en_lista) {return string_equals_ignore_case(((t_semaforo*)sem_en_lista)->nombre,carpincho->esperando);}
	bool buscar_carpincho(void* carpincho_en_lista) {return ((t_carpincho*)carpincho_en_lista)->pid == carpincho->pid;}

	t_semaforo* sem = list_find(lista_semaforos,buscar_bloqueando);
	list_remove_by_condition(sem->bloqueando,buscar_carpincho);
	list_add(sem->tomado_por, carpincho);
	list_add(carpincho->tomando, sem);
	responder(carpincho->instruccion->instruccion, EXITO, carpincho->socket_carpincho);
	carpincho->esperando = "VACIO";
...
int informar_finalizado(t_carpincho* carpincho){
	...
	printf("CONEXION: %d \n", conexion);
	puts("2");
	if(conexion == -1){ //TODO cambiar por distinto a -1
...
void detectar_deadlock(){
	...
	while(1){
		usleep(tiempo*1000); //TODO espera activa
